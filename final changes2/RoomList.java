...
/**
 * The room list that contains all room in the hotel.
 *
 * @author SJK
 * @version 1.0
 */
public class RoomList {
    ...
    /**
     * Add the booking dates to room from a database file.
     *
    ...
    public void addBookingDatesToRoom(Database database) {
        for (int i = 0; i < database.getReservationListFromDatabase().size(); i++) {
            ...
            for (int j = 0; j < roomList.size(); j++) {
                if (roomList.get(j).getRoomNumber() == roomNum && roomList.get(j).getRoomType() == roomType) {
                    roomList.get(j).addDate(startDate, endDate);
                    break;
                }
            }
        }
     ...
     * @return the array list of available room
     */
